    ...
    <Group align="flex-start" gap="lg" wrap="nowrap" style={{ flexDirection: wide ? 'row' : 'column' }}>
      ...
      */}
      <Box style={{ flex: 1, minWidth: 0, width: wide ? undefined : '100%' }}>
        {/*
          `printable-report` — это не пометка «можно печатать», а выбор механизма печати, и без него
          рекомендация печаталась **пустым листом**. Правило «документ на один лист» гасит всё
          (`visibility: hidden`) на любой странице, где нет ни `printable-report`, ни
          `printable-flow`, и показывает обратно только `.printable-document`; страница, не назвавшая
          себя никак, исчезает целиком. Замер: PDF на 853 байта, то есть чистая страница.

          Из двух механизмов нужен именно отчёт: рекомендация — это двести тысяч знаков, десятки
          листов, а «один лист» кладёт документ `position: fixed` и обрезал бы его на первом. Отчёт
          же убирает лишнее из потока, распускает высоту прокручиваемой оболочки, повторяет шапку
          таблицы на каждой странице и не рвёт строку между листами — всё это рекомендации нужно.
        */}
