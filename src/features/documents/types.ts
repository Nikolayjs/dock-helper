...
export interface DocumentSheet {
  ...
  /**
   * Высота строк данных в пунктах; отсутствующая считается по содержимому.
   *
   * Массив идёт параллельно `rows`, поэтому обязан ездить вместе с ними: вставка, удаление и
   * сортировка двигают строки, и высота, оставшаяся на месте, досталась бы соседке — ровно та же
   * беда, из-за которой форматы проводятся через `remapFormats`.
   */
