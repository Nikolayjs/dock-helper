...
describe('блокировка по бездействию', () => {
  ...
  describe('срок', () => {
    it('по умолчанию — рабочие сутки, и это настоящее число, а не «не задано»', () => {
      expect(readIdleMinutes()).toBe(DEFAULT_IDLE_MINUTES);
      expect(DEFAULT_IDLE_MINUTES).toBe(1024);
    });

    /*
     * Значение по умолчанию обязано быть среди вариантов: читатель отвергает всё, чего нет в списке,
     * и выпавшее из него значение отбрасывало бы само себя — настройка молча не сохранялась бы.
     */
    it('значение по умолчанию есть среди вариантов выбора', () => {
      expect(IDLE_LOCK_CHOICES as readonly number[]).toContain(DEFAULT_IDLE_MINUTES);
    });
  ...
  describe('подписи', () => {
    ...
    /* «Через 1024 мин» врач в уме не переведёт, а выбирает он именно по часам. */
    it('долгий срок подписан часами, и точное число стоит рядом', () => {
      expect(idleLockLabel(1024)).toBe('Через 17 ч (1024 мин)');
    });
