...
export function getTodayChecklist(notes: Note[]): ChecklistItem[] {
  ...
  return notes
    ...
    .flatMap((note) => note.items.map((item) => ({ ...item, noteId: note.id, noteTitle: note.title })));
}
...
export function getUpcomingReminders(patients: Patient[], limit: number): UpcomingReminder[] {
  return patients
    .filter((patient): patient is Patient & { reminderDate: string } => Boolean(patient.reminderDate))
...
export function getReminderStatusBreakdown(patients: Patient[]) {
  ...
  return (Object.keys(counts) as ReminderStatus[])
    ...
    .map((status) => ({ name: REMINDER_STATUS_LABEL[status], value: counts[status], color: REMINDER_STATUS_COLOR[status] }));
}
...
export function getReferralPeriodRange(period: ReferralPeriod): { start: string; end: string } {
  ...
  if (period === 'quarter') {
    ...
    return { start: start.format('YYYY-MM-DD'), end: start.add(3, 'month').subtract(1, 'day').format('YYYY-MM-DD') };
  }
  if (period === 'halfYear') {
    const start = now.startOf('month').subtract(now.month() % 6, 'month');
    return { start: start.format('YYYY-MM-DD'), end: start.add(6, 'month').subtract(1, 'day').format('YYYY-MM-DD') };
  }
...
export function getReferralBreakdown(patients: Patient[], start: string, end: string) {
  ...
  for (const patient of patients) {
    for (const visit of patient.visits) {
      ...
      const label = visit.referralDestination.trim() || REFERRAL_CATEGORY_LABELS[visit.referralCategory];
...
export function getReferralEntries(patients: Patient[], start: string, end: string, limit?: number): ReferralEntry[] {
