...
/*
 * Конспекты клинических рекомендаций, написанные самим врачом.
 *
 * Раньше это были заводские карточки-справки вида `guideline` — вида такого больше нет: раздел
 * «Клинические рекомендации» стал зеркалом рубрикатора Минздрава, и живёт оно на сервере, то есть в
 * демо недоступно, как и справочник МКБ-10. А статьи демо показывать обязано — ради них раздел и
 * открывают, — и конспект рекомендации это ровно то, что врач пишет себе сам.
 */
const article = (id: string, title: string, summary: string, tags: string[], body: string) => ({
  id,
  kind: 'article',
  title,
  summary,
  tags,
  author: DEMO_DOCTOR.name,
  content: body,
