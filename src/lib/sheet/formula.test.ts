...
describe('literalValue', () => {
  ...
  it('запятая — это дробь, а не текст', () => {
    // Ячейка — обычное поле ввода, и по-русски дробь набирают через запятую.
    expect(literalValue('1,5')).toBe(1.5);
    expect(literalValue('-2,25')).toBe(-2.25);
    expect(literalValue('1,5,5')).toBe('1,5,5');
  });
});
...
describe('арифметика', () => {
  ...
  it('смешанная колонка с запятыми складывается так же, как через плюс', () => {
    // До правки `=СУММ` видел ноль, а `=B2+B3` — четыре: агрегаты отбирают числа, а сложение
    // приводит текст к числу само. Разница ничем себя не выдавала.
    const mixed = sheet([
      ['Объём'],
      ['1,5'],
      ['2.5'],
    ...
    expect(at(mixed, 'A4')).toBe('4');
    expect(at(mixed, 'A5')).toBe('4');
    expect(at(mixed, 'A6')).toBe('2');
  });
