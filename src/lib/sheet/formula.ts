...
/**
 * Число, которым ячейка становится — одно правило на весь путь: и в вычислении, и в выгрузке.
 *
 * Осторожное: `007` и `89123456789` остаются текстом, иначе формула складывала бы телефоны, а у
 * номера пропадал бы ведущий ноль. Дата (`12.09.2026`) числом не становится тоже.
 *
 * **Запятая принимается наравне с точкой.** Ячейка таблицы — обычное поле ввода, и по-русски дробь
 * набирают через запятую; без этого правила `1,5` оставалось текстом, и `=СУММ(B2:B3)` над двумя
 * такими ячейками давало **0**, тогда как `=B2+B3` рядом давало 4: сложение приводит текст к числу
 * само. Ноль без ошибки, молча, — и он же уходил на печать и в .xlsx.
 *
 * Правило обязано быть **одним** и для выгрузки: отдай мы `1,5` текстом, Excel пересчитал бы нашу
 * же формулу по своим правилам и показал в файле 0 там, где на экране было 4.
 */
 ...
 *
 * Пустая ячейка — пустая строка, а не ноль: `СЧЁТ` не должен их считать.
 */
export function literalValue(raw: string): CellValue {
  const trimmed = raw.trim();
  if (trimmed === '') return '';
  return cellNumber(trimmed) ?? trimmed;
}
